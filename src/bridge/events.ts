import bridge, { EventCallback } from "@rbxts/bridge";
...
export const useEvent: {
  ...
  if (callback !== undefined) {
    ...
    onCleanup(() => bridge.off(event, callback));
  }
  return [
    (sent: unknown, player?: Player) => {
      bridge.send(event, sent, player);
    }, RunService.IsServer() ? (sent: unknown) => {
      bridge.broadcast(event, sent);
    } : undefined
  ] as EventCallers<string>;
}