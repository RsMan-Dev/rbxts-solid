import { FunctionCallback } from "@rbxts/bridge";
type FunctionCallers<T extends string | keyof BridgeFunctionMap> = {
    async: T extends keyof BridgeFunctionMap ? (data: BridgeFunctionMap[T][0], player?: Player) => Promise<BridgeFunctionMap[T][1]> : (data: unknown, player?: Player) => Promise<unknown>;
    call: T extends keyof BridgeFunctionMap ? (data: BridgeFunctionMap[T][0], player?: Player) => BridgeFunctionMap[T][1] : (data: unknown, player?: Player) => unknown;
};
/**
 * # useFunction
 ...
 */
export declare const useFunction: {
